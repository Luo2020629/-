...
# 设置token
ts.set_token('262474d8ae99f188cdd6cbb167d609535f77f4f144ca5b1e3a558a30')  # 请替换为你的实际token
pro = ts.pro_api()
...
def display_results(result_df, max_display=15):
    ...
    # 定义显示列的顺序（优先显示重要信息）
    preferred_columns = ['股票代码', '交易日期', '开盘价', '最高价', '最低价', '收盘价', 
                        '涨跌幅', '涨跌额', '成交量', '成交额', '昨收价']
    ...
    if '涨跌幅' in result_df_chinese.columns:
        ...
        print(f"最大涨跌幅: {result_df_chinese['涨跌幅'].max():.2f}%")
        print(f"最小涨跌幅: {result_df_chinese['涨跌幅'].min():.2f}%")
        print(f"上涨股票数量: {len(result_df_chinese[result_df_chinese['涨跌幅'] > 0])}")
        print(f"下跌股票数量: {len(result_df_chinese[result_df_chinese['涨跌幅'] < 0])}")
